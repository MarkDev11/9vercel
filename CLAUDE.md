...
- Model self-test (`POST /api/models/test` → `pingModelByKind` in `src/app/api/models/test/ping.js`) defaults `baseUrl` to loopback `http://127.0.0.1:${PORT||20128}` — dead on Vercel serverless (no loopback listener), surfacing as `fetch failed` 500 on provider-detail Test buttons while Suggested-models (direct upstream fetch) still loads. Any fix must avoid loopback self-fetch on Vercel (in-process handler call or absolute app URL), not just tune timeouts.
