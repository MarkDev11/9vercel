...
- Port-gate convention (established 0.5.69→0.5.81): after each version, `npm run build` green + full vitest fail-set diffed against the `v0.5.65` baseline (`125` fails) — signal is port-only new fails. Known upstream-stale fails left red, never "fixed" by weakening the port: `unit/kiro-external-idp.test.js` endpoint ordering (upstream reordered to `q.*` first, never updated the test), and `it.fails` bug-exposure tests flip to `it` when the ported fix resolves them (0.5.81 flipped 2 image-preservation cases, updated 3 CommandCode `mediaType` expectations, re-asserted the CommandCode error-throw).
