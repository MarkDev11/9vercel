...
export async function initializeApp() {
  try {
    // On Vercel: skip OS-level sidecars (cloudflared, MITM DNS, mcp bridges).
    // Keep only DB bootstrap + background token refresh (via cron instead of interval).
    if (IS_VERCEL) {
      if (!g.signalHandlersRegistered) g.signalHandlersRegistered = true;
      // Still run light startup: DB migration + cleanup (no tunnel/MITM/quota ping).
      setTimeout(() => {
        runHeavyStartupVercel().catch((e) => console.error("[InitApp:Vercel] startup failed:", e.message));
      }, 500);
      return;
    }
...
async function runHeavyStartupVercel() {
  // Minimal startup for serverless: ensure DB reachable and prune old connections.
  try {
    await cleanupProviderConnections();
  } catch (e) {
    console.warn("[InitApp:Vercel] cleanupProviderConnections skipped:", e.message);
  }
  // Background token refresh is handled by Vercel Cron (/api/cron/refresh-tokens),
  // not by an in-process interval (serverless functions don't stay alive).
  if (process.env.DISABLE_BACKGROUND_TOKEN_REFRESH === "1" || process.env.DISABLE_BACKGROUND_TOKEN_REFRESH === "true") return;
  // No-op: cron will invoke runBackgroundTokenRefreshTick.
}
