...
// NOTE (fork): proxyPoolFitness (markPoolUnfit/clearPoolUnfit) is not ported —
// pool-scoped errors below still carry `poolScoped` for forward-compat, and the
// per-account / per-proxy cooldown maps in this file provide the same pacing.
 ...
 *
 * Wire shape mirrors the official CLI exactly: the body carries top-level
 * `codebuff_metadata` ({ run_id, client_id, cost_mode, freebuff_instance_id? })
 * and `provider: { allow_fallbacks }` — NOT nested under a `codebuff` object
 * (the backend rejects the nested shape with 400 "No runId found in request body").
 *
 * The run_id is not free-form: the backend resolves it against its agent-run
 * store, so every chat request first registers a run via POST /api/v1/agent-runs
 * ({ action:"START", agentId, ancestorRunIds:[] }) → { runId }. The free tier
 * additionally gates on a session: POST /api/v1/freebuff/session with an
 * `x-freebuff-model` header claims a row (bound to one model, ~1h).
 */
...
// picker row. A claim must peek at the current offers first instead of
// POSTing blind. Offer state is per-account and cached briefly.
...
// the canonical Freebuff CLI root prompt (403 free_mode_cli_required).
const FREEBUFF_SYSTEM_MARKER = "You are Buffy, the strategic coding assistant.";

// Canonical openings accepted by the server gate. The check is a byte-exact
// prefix on the first message, so our injected marker must be verbatim.
...
// The backend's foreign_toolset gate rejects tool-calling requests whose
// toolset lacks the CLI's `end_turn` tool with a misleading 404 "No endpoints
// found for {model}". Every request that declares tools must carry it.
const END_TURN_TOOL = {
  type: "function",
  function: {
    name: "end_turn",
...
function injectEndTurnTool(body) {
  ...
  const hasEndTurn = tools.some((t) => t?.function?.name === "end_turn");
  if (hasEndTurn) return body;
  return { ...body, tools: [...tools, END_TURN_TOOL] };
}

// Freebuff root agent id per model. Withdrawn upstream models are deliberately
// absent: no new session can be admitted on them.
...
// Per-token+model session cache (in-memory). All state lives on globalThis so
// Next dev (Turbopack) bundles share ONE copy.
...
function throwSessionGateError(gate, { token, model, proxyKey, log }) {
  if (gate.kind === "model_locked") {
    ...
    log?.warn?.("AUTH", `Freebuff model_locked (session=${label}, requested=${model})`);
    throw err;
  }
  if (gate.kind === "limited_ip") {
    ...
    err.status = 409;
    err.poolScoped = { poolId: null, scope, reason: "limited_ip" };
    log?.warn?.("AUTH", `Freebuff limited-IP refused ${model}`);
    throw err;
  }
}
...
async function guardOfferClaim(token, model, proxyOptions) {
  ...
  if (!offer || Number(offer.remaining) <= 0) {
    const err = new Error(
      "Claude Fable 5 is not being offered right now — it is a capacity-limited trial served in waves. Retry later.",
    );
    err.status = 409;
  ...
  if (Number.isFinite(userLeft) && userLeft <= 0) {
    ...
    const err = new Error("Your Freebuff account has used its Claude Fable 5 sessions for today.");
    err.status = 409;
...
export class FreebuffExecutor extends BaseExecutor {
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    // reasoning_effort collides → 400. Mirror the CLI: send none.
  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null }) {
    ...
    if (poolUntil) {
      ...
      err.status = 409;
      err.poolScoped = { poolId: proxyOptions?.proxyPoolId || null, scope, reason: "limited_ip" };
      throw err;
    }
    ...
    try {
      ...
      if (gate) throwSessionGateError(gate, { token, model, proxyKey, log });
    ...
    try {
      ...
      if (SESSION_STALE_CODES.has(response.status)) {
        ...
        if (gate.kind === "model_locked" || gate.kind === "limited_ip") {
          ...
          throwSessionGateError(gate, { token, model, proxyKey, log });
        }
        ...
        try {
          ...
          if (gate2) throwSessionGateError(gate2, { token, model, proxyKey, log });
        ...
        if (SESSION_STALE_CODES.has(response.status)) {
          ...
          if (gate3.kind === "model_locked" || gate3.kind === "limited_ip") {
            throwSessionGateError(gate3, { token, model, proxyKey, log });
          }
          const err = new Error(
